...
const App: React.FC = () => {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    setViewState(ViewState.HOME); // Ensure we go back to home when searching
    ...
    try {
      ...
      const aiFilters = await parseSearchQuery(searchQuery);
      setActiveFilters(aiFilters);
      
      // Filter logic based on AI extraction + basic keyword matching
      const filtered = MOCK_ROOMS.filter(room => {
        let match = true;
        
        // 1. City Match (AI extracted)
        if (aiFilters.city && !room.city.toLowerCase().includes(aiFilters.city.toLowerCase())) {
          match = false;
        }

        // 2. Price Match (AI extracted)
        if (aiFilters.maxPrice && room.pricePerWeek > aiFilters.maxPrice) {
          match = false;
        }

        // 3. Keyword/Amenity Match (AI extracted or raw text fallback)
        const keywords = aiFilters.keywords && aiFilters.keywords.length > 0 
          ? aiFilters.keywords 
          : searchQuery.toLowerCase().split(' ');

        // Check if ANY keyword matches room description, title, or amenities
        ...
        // Only enforce keyword match if specific keywords were found/typed
        if (keywords.length > 0 && !keywordMatch && !aiFilters.city && !aiFilters.maxPrice) {
            // If we have no city/price filter, strictly rely on text match
            match = false;
        }
  ...
  const displayRooms = viewState === ViewState.SAVED 
    ? MOCK_ROOMS.filter(r => savedRoomIds.includes(r.id))
    : rooms;
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 pb-16 w-full mt-10">
        ...
        {viewState === ViewState.SAVED && (
           <div className="mb-8 border-b border-slate-200 pb-4">
              ...
              <p className="text-slate-500 mt-2">
                 {savedRoomIds.length === 0 ? "You haven't saved any properties yet." : `You have ${savedRoomIds.length} saved properties.`}
              </p>
           </div>
        )}
        ...
        {viewState === ViewState.BOOKING_SUCCESS ? (
          ...
          <>
            ...
            {viewState === ViewState.SAVED && displayRooms.length === 0 && (
               <div className="text-center py-20">
                  <button 
                    onClick={() => setViewState(ViewState.HOME)}
                    className="bg-brand-600 text-white px-6 py-3 rounded-full font-medium hover:bg-brand-700 transition"
                  >
                    Browse Listings
                  </button>
               </div>
      ...
      {/* Logic to show Details even when coming from Saved view */}
